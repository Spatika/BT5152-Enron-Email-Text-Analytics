...
```{r}
doc2vec_cluster_labels = read.csv("cluster_labels.csv", stringsAsFactors = FALSE, header = F)
```
...
combined_features_df <- cbind(combined_features_df, doc2vec_cluster_labels, document_topic_dist, dict_features)
...
mean(pred_knn_tr == train_poi$poi) # 0.966

# Testing accuracy
pred_knn <- predict(model_knn,newdata = test_features_df)
mean(pred_knn == test_poi$poi) # 0.837
...
as.numeric(performance(test_pred_knn, "auc")@y.values) # 0.48 auc
...
confusionMatrix(train_cm_rpart, mode = 'everything', positive = "True") # acc 0.9213, prec 0.6316, recall 1.0

test_cm_rpart <- table(predict(rpart_model_smote, test_features_df), test_poi$poi)
confusionMatrix(test_cm_rpart, mode = 'everything', positive = "True") # acc 0.7027, pre 0.2, recall 0.4

# AUC
test_pred_rpart <- ROCR::prediction(as.numeric(predict(rpart_model_smote, test_features_df)), as.numeric(test_poi$poi))
as.numeric(performance(test_pred_rpart, "auc")@y.values) # 0.575 auc
...
# accuracy 0.78, prec 0.33, recall 0.60

# AUC
test_pred_svm <- ROCR::prediction(as.numeric(pred_svm), as.numeric(test_poi$poi))
as.numeric(performance(test_pred_svm, "auc")@y.values) # 0.6 auc

```

## 3C - Ensemble : Random Forest
Improves accuracy to 0.89 from best 3B model.
```{r}
set.seed(5152)
library(randomForest)
rf_model <- train(poi ~ ., data = cbind(train_features_df,train_poi), 
...
# acc 0.89, prec 1.0, recall 0.2

# AUC
test_pred_rf <- ROCR::prediction(as.numeric(predict(rf_model, test_features_df)), as.numeric(test_poi$poi))
as.numeric(performance(test_pred_rf, "auc")@y.values) # 0.6 auc
```
...
as.numeric(performance(test_pred_xgb, "auc")@y.values) # 0.88 auc
```
...
#tuning with cost parameter
model_svm_fe <- train(factor(poi) ~., data = cbind(train_features_df2, train_poi),
                    metric = "ROC", method = "svmLinear", 
                    trControl = control, tuneGrid = expand.grid(.C= c(1,2,0.5)))
 
# training accuracy 0.943
pred_svm_tr <- predict(model_svm_fe, train_features_df2)
mean(pred_svm_tr == train_poi$poi)
confusionMatrix(pred_svm_tr, train_poi$poi, mode = 'everything', positive = 'True')

# testing accuracy 0.837
pred_svm_fe <- predict(model_svm_fe, test_features_df2)
mean(pred_svm_fe == test_poi$poi) # 0.837
confusionMatrix(pred_svm_fe, test_poi$poi, mode = 'everything', positive = 'True')
