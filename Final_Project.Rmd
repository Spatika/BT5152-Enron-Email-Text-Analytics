...
cm$byClass["Precision"] # 0.92
cm$byClass["Recall"] # 0.782

# AUC
test_pred_knn <- ROCR::prediction(as.numeric(pred_knn), as.numeric(test_poi$poi))
as.numeric(performance(test_pred_knn, "auc")@y.values) # 0.591 auc
...
rpart_model_smote <- train(poi ~ ., data = cbind(train_features_df, train_poi), method = "rpart", metric = "ROC", trControl = control, preProcess = c("scale", "center"), tuneGrid = grid)
...
```{r}
model.svm <- svm(factor(poi) ~., data = train2, kernel = "linear")

#Training accuracy 0.96

pred_svm <- predict(model.svm, train_features_df)
mean(pred_svm == train_poi$poi)

#Testing accuracy 0.82
pred_svm <- predict(model.svm, test2)
mean(pred_svm ==  test_poi$poi)
cm = confusionMatrix(pred_svm,  test_poi$poi)

cm$byClass["Precision"] #0.815
cm$byClass["Recall"] #1
```
