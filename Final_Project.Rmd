...
# correlation between topics and POI/not
...
# AUC
test_pred_svm <- ROCR::prediction(as.numeric(pred_svm), as.numeric(test_poi$poi))
as.numeric(performance(test_pred_svm, "auc")@y.values) # 0.7347 auc
```
...
grid <- expand.grid(.nrounds=c(25, 50, 100, 200, 500),.eta=c(0.005, 0.01, 0.05, 0.1),.gamma=c(0,1),
                     .max_depth=c(2,3,4,5),.colsample_bytree=c(0.6, 0.8, 1),
                     .subsample=c(0.5, 0.75, 1),.min_child_weight=c(1, 2, 3))
