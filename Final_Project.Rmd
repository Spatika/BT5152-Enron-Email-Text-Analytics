...
```
## Feature from Python Word2Vec Clustering
```{r}
cluster_labels = read.csv("cluster_labels.csv", stringsAsFactors = FALSE, header = F)
```
...
combined_features_df <- cbind(combined_features_df, tf_idf_features, document_topic_dist, cluster_labels)
...
# AUC
test_pred_svm <- ROCR::prediction(as.numeric(pred_svm), as.numeric(test_poi$poi))
as.numeric(performance(test_pred_svm, "auc")@y.values) # 0.63 auc


```

## 3C - Random Forest - improves recall to 0.934
```{r}
...
# 3B model - SVM accuracy before - 0.84

# 3C model - stacked model before - 
